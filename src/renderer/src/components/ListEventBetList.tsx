...
const defaultColumns = [
  ...
  },
  {
    ...
    size: 50,
    cell: ({ getValue }) => {
      return <span className="text-red-600"> {getValue() as string}</span>
    }
  },
  {
    accessorKey: 'speed',
...
export const ListEventBetList = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const data = await window.electron.ipcRenderer.invoke('GetBetList')
  ...
  useEffect(() => {
    const handleDataBetList = (_, data: { id: number; idPair: string; dataPair: string }) => {
      setData((prevState) => {
    ...
    window.electron.ipcRenderer.on('DataBetList', handleDataBetList)
